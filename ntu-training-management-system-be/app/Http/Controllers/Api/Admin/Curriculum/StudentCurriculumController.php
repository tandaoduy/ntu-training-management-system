...
class StudentCurriculumController extends Controller
{
    ...
    public function versions(Request $request): JsonResponse
    {
        $payload = $request->validate([
            'nganh_dao_tao_id' => ['sometimes', 'integer', 'exists:nganh_dao_taos,id'],
            'trang_thai' => ['sometimes', Rule::in(['draft', 'published', 'archived'])],
        ]);

        $versions = PhienBanCtdt::query()
            ->with([
            ...
            ->when(isset($payload['nganh_dao_tao_id']), fn ($query) => $query
                ->whereHas('chuongTrinhDaoTao', fn ($ctdtQuery) => $ctdtQuery
                    ->where('nganh_dao_tao_id', (int) $payload['nganh_dao_tao_id'])))
            ->when(isset($payload['trang_thai']), fn ($query) => $query
                ->where('trang_thai', $payload['trang_thai']))
            ->orderByDesc('id')
            ->get()
            ->map(fn (PhienBanCtdt $version): array => $this->versionPayload($version))
            ->values();
    ...
    public function programs(Request $request): JsonResponse
    {
        ...
        $programs = ChuongTrinhDaoTao::query()
            ->with([
                'nganhDaoTao:id,ma_nganh,ten_nganh',
                'phienBans' => fn ($query) => $query
            ...
            ->orderBy('ma_ctdt')
            ->get()
            ->map(fn (ChuongTrinhDaoTao $program): array => $this->programPayload($program))
            ->values();
    ...
    public function programDetail(ChuongTrinhDaoTao $chuongTrinhDaoTao): JsonResponse
    {
        $chuongTrinhDaoTao->load([
            'nganhDaoTao:id,ma_nganh,ten_nganh',
            'phienBans' => fn ($query) => $query
                ->where('trang_thai', 'published')
                ->orderByDesc('version_no'),
        ...
        $version = $chuongTrinhDaoTao->phienBans->first();

        if (! $version) {
            return $this->jsonResponse([
                'message' => 'Chương trình đào tạo chưa có phiên bản đã công bố.',
                'data' => null,
            ], 404);
        }

        $version->setRelation('chuongTrinhDaoTao', $chuongTrinhDaoTao);

        return $this->jsonResponse([
            'data' => [
                'program' => $this->programPayload($chuongTrinhDaoTao),
                'curriculum' => $this->curriculumPayload($version),
            ],
        ]);
    }

    public function versionDetail(PhienBanCtdt $phienBanCtdt): JsonResponse
    {
        $phienBanCtdt->load([
            'chuongTrinhDaoTao:id,ma_ctdt,ten_ctdt,nganh_dao_tao_id,tong_tin_chi_yeu_cau',
            'chuongTrinhDaoTao.nganhDaoTao:id,ma_nganh,ten_nganh,he_dao_tao,thoi_gian_dao_tao',
        ...
        return $this->jsonResponse([
            'data' => $this->curriculumPayload($phienBanCtdt),
        ]);
    }
    ...
    public function show(SinhVien $sinhVien): JsonResponse
    {
        ...
        return $this->jsonResponse([
            'data' => $assignment ? $this->assignmentPayload($assignment) : null,
        ]);
    }

    public function store(Request $request, SinhVien $sinhVien): JsonResponse
    {
        $payload = $request->validate([
        ...
        $version = PhienBanCtdt::query()
            ->with('chuongTrinhDaoTao:id,nganh_dao_tao_id')
            ->findOrFail((int) $payload['phien_ban_ctdt_id']);

        $studentMajorId = (int) ($sinhVien->nganh_dao_tao_id ?? 0);
        $curriculumMajorId = (int) ($version->chuongTrinhDaoTao?->nganh_dao_tao_id ?? 0);

        if ($studentMajorId > 0 && $curriculumMajorId > 0 && $studentMajorId !== $curriculumMajorId) {
            return $this->jsonResponse([
                'message' => 'Phiên bản CTĐT không thuộc ngành đào tạo của sinh viên.',
                'errors' => [
                    'phien_ban_ctdt_id' => ['Phiên bản CTĐT không phù hợp với ngành đào tạo của sinh viên.'],
                ],
            ], 422);
        }
        ...
        $assignment = DB::transaction(function () use ($sinhVien, $payload, $locked): SinhVienChuongTrinh {
            if ($locked) {
                SinhVienChuongTrinh::query()
                    ->where('sinh_vien_id', $sinhVien->id)
                    ->where('phien_ban_ctdt_id', '!=', (int) $payload['phien_ban_ctdt_id'])
                    ->update(['locked' => false]);
            }

            return SinhVienChuongTrinh::query()->updateOrCreate(
                [
                    'sinh_vien_id' => $sinhVien->id,
                    'phien_ban_ctdt_id' => (int) $payload['phien_ban_ctdt_id'],
                ],
                [
                    'ngay_ap_dung' => $payload['ngay_ap_dung'] ?? now()->toDateString(),
                    'locked' => $locked,
        ...
        return $this->jsonResponse([
            'message' => 'Đã gán chương trình đào tạo cho sinh viên.',
    ...
    private function programPayload(ChuongTrinhDaoTao $program): array
    {
        return [
            'id' => $program->id,
            'ma_ctdt' => $program->ma_ctdt,
            'ten_ctdt' => $program->ten_ctdt,
            'tong_tin_chi_yeu_cau' => $program->tong_tin_chi_yeu_cau,
            'nganh_dao_tao' => $program->nganhDaoTao,
            'versions' => $program->relationLoaded('phienBans')
                ? $program->phienBans->map(fn (PhienBanCtdt $version): array => $this->versionPayload($version))->values()
                : [],
        ];
    }

    private function curriculumPayload(PhienBanCtdt $version, ?array $termFilter = null, ?SinhVien $student = null): array
    {
        ...
        $itemsByGroupId = DB::table('phien_ban_ctdt_hoc_phans as pivot')
            ...
            ->when($termFilter, function ($query) use ($termFilter): void {
                if (($termFilter['mode'] ?? null) === 'odd') {
                    $query->whereNotNull('pivot.hoc_ky_goi_y')
                        ->whereRaw('MOD(pivot.hoc_ky_goi_y, 2) = 1');
                }

                if (($termFilter['mode'] ?? null) === 'even') {
                    $query->whereNotNull('pivot.hoc_ky_goi_y')
                        ->whereRaw('MOD(pivot.hoc_ky_goi_y, 2) = 0');
                }

                if (($termFilter['mode'] ?? null) === 'summer') {
                    $query->whereNull('pivot.hoc_ky_goi_y');
                }
            })
            ->select(array_filter([
    ...
    private function studentTermFilter(): ?array
    {
